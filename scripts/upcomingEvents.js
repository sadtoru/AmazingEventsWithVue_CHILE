...
const app = createApp({
  ...
  data() {
    return {
      ...
      showUpcoming: [], // array con los eventos futuros a mostrar
    ...
    getData() {
      fetch(this.getEvents) // hacemos una petición a la URL declarada arriba
        ...
        .then(data => {
          ...
          this.showUpcoming = this.events.filter(event => event.date > currentDate)
  ...
  computed: {
    ...
    superFilter() {
      ...
      let filteredEvents = this.showUpcoming.filter(event => event.name.toLowerCase().includes(this.text.toLowerCase()))
